...
void QuestionDialogManager::post(
				char*          message,
                                char*          title,
                                void*          clientData,
				...
				int	       cancelBtnNum)
{
  //  Widget dialog = this->getDialog(parent);
    ...
    this->DialogManager::post( message, title, clientData,
...
void QuestionDialogManager::createDialog(void)
{
    //Widget dialog;
...
int QuestionDialogManager::userQuery (char* message, 
...
//XtAppContext app = theApplication->getApplicationContext();
    ...
    this->modalPost (message, title, (void*)&users_response,
