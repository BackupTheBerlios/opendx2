...
_dxf_CopyBufferToXImage(struct buffer *buf, Field image, int xoff, int yoff)
{
    ...
    return _dxf_translateImage((Object)image, iwidth, iheight,
		xoff, yoff, buf->ox, buf->oy, buf->width, buf->height,
		0, 1, translation,
		src, inType, dst);
#else
    return _dxf_translateImage((Object)image, iwidth, iheight,
...
#ifdef DXD_BIGENDIAN
#define COLORMASK	0xfff0
#else
#define COLORMASK	0x0fff
#endif
...
#ifdef DXD_BIGENDIAN
#define COLORMASK	(XImageByteOrder(t->dpy) == MSBFirst) ? 0x00ffffff : 0xffffff00
#else
#define COLORMASK	(XImageByteOrder(t->dpy) == MSBFirst) ? 0xffffff00 : 0x00ffffff
#endif
...
_dxf_DecodeImage(Object encodedimage)
{
	...
	{
	    DXSetError(ERROR_INVALID_DATA, "invalid image");
	    return NULL;
	}
	...
	{
	    DXSetError(ERROR_INVALID_DATA, "invalid encoded image");
	    return NULL;
	}
