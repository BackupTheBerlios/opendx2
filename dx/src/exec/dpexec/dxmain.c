...
char *_dxd_exHostName;
int  _dxd_exPPID;		 /* parent's process id */
int *_dxd_exTerminating;	 /* flag set when dx is exiting */
int _dxd_exSelectReturnedInput = FALSE;  /* flag set when select returned from yyin */
Context *_dxd_exContext;         /* structure for context information */
int  _dxd_exMyPID;		 /* pid of the process */
int  _dxd_exMasterfd = -1;       /* slave to master file descriptor */
int  _dxd_exSlaveId;             /* slave number */
int  _dxd_exSwapMsg;             /* do we need to swap msg from peer? */
...
static void ExProcessArgs (int argc, char **argv)
{
	...
	switch (opt)
	{
	    case 'B': _dxd_exShowBells = TRUE;			break;
	    case 'C': extestport = atoi(optarg); 		break;
	    case 'D': _dxd_exDebugConnect = TRUE; 		break;
	    case 'E': _dxd_exErrorPrintLevel = atoi (optarg);	break;
	    case 'F': if (optarg == NULL) {
		      ...
		      break;	  
            case 'H': strcpy(extesthost, optarg); 		break; 
	    case 'L':
#ifdef DXD_LICENSED_VERSION
	    ...
	    case 'X': _dxd_exSkipExecution = TRUE;		break;
	    case 'V': _dxd_exEnableDebug = TRUE;		break;

	    case 'c': _dxd_exCacheOn = FALSE;			break;
	    case 'd': _dxd_exDebug = TRUE;			break;
	    case 'h': ExUsage (argv[0]);			break;
	    case 'i': read_i_threshhold = atof (optarg);	break;
	    case 'l': logcache = TRUE;				break;
	    case 'm': MarkModules = TRUE;			break;
	    case 'p':
		if (optarg == NULL      ||
		    ! isdigit (*optarg) ||
	    ...
	    case 'r': _dxd_exRemote = TRUE;			break;
            case 's': _dxd_exRemoteSlave = TRUE;                break;
	    case 't': _dxd_exShowTiming = 1;		        break;
	    case 'u': _dxd_exParseAhead = FALSE; 		break;
	    case 'v': ExVersion ();				break;

	    default : ExUsage (argv[0]);			break;
	}
    }
}
...
static void ExCleanup ()
{
    ...
    ok = (exParent || (nprocs == 1 && _dxd_exMyPID == 0 && ! processor_status_on));
