...
#if defined(sun4) || defined(solaris) || defined(sgi) || defined(alphax) || defined(aviion) || defined(linux86)
 ...
 *
 *  -bI:dxexec.exp   this lists the symbols in the exec which your module
 ...
 *
 * -e _entrypoint    this sets the name of the function which will be returned
 *                   as the entry point - it must be the symbol DXEntry, which
 *                   is a routine generated by the mdf2c script when building
 *                   the module or package of modules.
 *
 * the exec doesn't have to be linked with any special flags because it
 *  isn't expecting to call any of the subroutines defined in these modules
 *  directly - it is only going to call the single entry point, and it makes
 *  that call by address and not by name.
 *
...
PFI DXLoadObjFile(char *fname, char *envvar)
{
    ...
    DXDebug("L", "setting tp entry for `%s'", foundname);
    if (!set_tp_entry(L_LOADED, foundname, func, func)) {
	DXFree((Pointer)foundname);
	DXunlock(ltl, 0);
	return NULL;
    }
    DXDebug("L", "tp entry now set for `%s'", foundname);
