...
macro	:	K_MACRO T_ID T_LPAR formal_s0 T_RPAR 
		...
		attributes
         	block { Network.ParseState.network.ParseEndOfMacroDefinition();}
	;
...
list returns [string l_val="{"] {string r_val, s_val;}
...
list_r returns [string l_val=""] {string r_val;}
                : T_RBRA {l_val="}";}
                | T_COLON r_val=real T_RBRA {l_val=":"+r_val+"}";}
                ;
                                                                                
tensor_s returns [string ts_val=""] {string s_val="";}
                : ts_val = tensor
                  (s_val=tensor {ts_val+=" " + s_val;}
                  | T_COMMA s_val=tensor { ts_val += "," + s_val;})*
                ;
                                                                                
tensor returns [string t_val="["] {string s_val;}
                : T_LSQB s_val = scalar_s T_RSQB {t_val+=s_val + "]";}
                | T_LSQB s_val = tensor_s T_RSQB {t_val+=s_val + "]";}
                ;
                                                                                
scalar_s returns [string ss_val=""] {string s_val;}
                : ss_val = scalar
                  (s_val = scalar {ss_val += " " + s_val;}
                  | T_COMMA s_val = scalar {ss_val += "," + s_val;})*
                ;
                                                                                
scalar  returns [string s_val=""] {string c_val="";}
                : s_val = real
                | T_LPAR c_val = complex T_RPAR {s_val="("+c_val+")";}
                ;
                                                                                
real returns [string r_val=""] 
                : r_val = int_type
                | r_val = float_type
                ;
                                                                                
complex returns [string c_val=""] {string r_val, q_val=""; }
...
quaternion_i returns [string q_val=","] {string val1, val2;}
                : T_COMMA val1 = int_type   T_COMMA  val2 = int_type
                  { q_val += val1 + "," + val2;}
                ;
                                                                                
quaternion_f returns [string q_val=","] {string val1, val2;}
                : T_COMMA val1=float_type T_COMMA val2=float_type
                  { q_val += val1 + "," + val2;}
                ;
                                                                                
                                                                             
int_type returns [string r_val=""]
                : i_val:T_INT {r_val=i_val.getText();}
                ;
                                                                                
float_type returns [string r_val=""]
                : f_val:T_FLOAT {r_val=f_val.getText();}
                ;
                                                                                
string_s returns [string ss_val=""] {string s_val;}
                : ss_val = stringv
                  (s_val = stringv {ss_val+=" "+s_val;}
                  | T_COMMA s_val = stringv {ss_val+=","+s_val;} )*
                ;
                                                                                
stringv returns [string n_str=""]
                : str:T_STRING
                { 
                  n_str=str.getText().Substring(1,str.getText().Length-2);
