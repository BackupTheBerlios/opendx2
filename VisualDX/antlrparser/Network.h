...
class Network : public Base
{
  ...
  private:
    //
    // Methods for extern "C" functions reference by antlr. 
    //
    ...
    //
    friend void ParseComment(const char* comment);
    friend void ParseFunctionID(const char* name);
    friend void ParseArgument(const char* name, const boolean isVarname);
    friend void ParseLValue(const char* name);
    friend void ParseRValue(const char* name);
    friend void ParseIntAttribute(const char* name, int value);
    friend void ParseStringAttribute(const char* name, const char* value);
...
private:
  ...
  protected:
    ...
    boolean		parseAntlr(std::ifstream& f);
    boolean		dirty;
    boolean		fileDirty;
    char*               fileName;
  ...
  public:
    ...
    boolean openCfgAntlr(const char *name, boolean openStartup = FALSE, 
					  boolean send = TRUE);
