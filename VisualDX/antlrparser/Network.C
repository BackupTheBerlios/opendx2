			     ...
			     boolean ignoreUndefinedModules)
{
    ...
    this->closeNetworkFILE(f);	//Will open ifstream for antlr
    ...
    std::ifstream infile(netfile);
    boolean parse_terminated = this->parseAntlr(infile); 
    ...
    this->openCfgAntlr(cfgfile, FALSE, FALSE);
...
boolean Network::parseAntlr(std::ifstream& input)
{
    boolean  result;
    ...
    try{
       NetLexer lexer(input);
       NetParser parser(lexer);
       parser.start();
       input.close();
    }
...
void Network::parseComment(const char* comment)
{
    ASSERT(comment);
...
void Network::parseFunctionID(const char *name)
{
...
void Network::parseArgument(const char* name, const boolean isVarname)
{
    int        parsed_items;
    int        instance;
    int        parameter;
...
void Network::parseLValue(const char* name)
{
    if (this->parseState.stop_parsing)
	return;
...
void Network::parseStringAttribute(const char* name, const char *value)
{
    if (this->parseState.stop_parsing)
	return;
...
void Network::parseIntAttribute(const char* name, int value)
{
...
void Network::parseRValue(const char* name)
{
    int        parsed_items;
    int        instance;
    int        parameter;
...
boolean Network::openCfgAntlr(const char *name, 
				boolean openStartup, boolean send)
{
    char *cfgfile;
    boolean ret;
...
#endif
        this->readingNetwork = TRUE;
        boolean stopped_parsing = this->parseAntlr(infile);
	if (openStartup)
	    this->openControlPanel(-1);
	if (send)
	    this->sendValues(FALSE);
